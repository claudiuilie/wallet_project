...
class mysqlController {
    ...
    constructor(config) {
     ...
     async getId(table, conditions){
        ...
        let q = `SELECT id FROM ${table} WHERE `;
        let p = Object.values(conditions);    
        
        for (let k in conditions) {
            q += `${k} = ?`;
            if (Object.keys(conditions).length > 1 && k !== Object.keys(conditions).pop()) {
                q += ' AND '
                }
            }

        this.connection.query(q, p, (error, res) => {
             id = res[0].id;
        })
    ...
    query(table,query,results) {
        ...
        switch(query[0]){
            ...
            case "SELECT":
                     q = `SELECT ${query[1].join()} FROM ${table} WHERE `;
...
var mysql2 = new mysqlController(config.mysql);

let xxx = mysql2.getId('income', {'year': 2019, 'month': 'January'});
